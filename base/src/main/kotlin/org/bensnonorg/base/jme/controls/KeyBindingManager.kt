package org.bensnonorg.base.jme.controls
...
import org.bensnonorg.base.kotlin.initOnce
...
class KeyBindingManager : Savable {
	...
	constructor(map: Map<String, Trigger>) {
		bindings = map.map { it.key to SavableTrigger(it.value) }.toMap(HashMap())
	}
	...
	class SavableTrigger : Savable {
		...
		internal var trigger: Trigger? = null
			set(value) {
				if (value != null)
					TriggerTypes.findType(value) ?: throw UnsupportedOperationException(
		...
		override fun write(ex: JmeExporter) {
			...
			val type = TriggerTypes.findType(trigger) ?: throw AssertionError()
